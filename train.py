...
parser.add_argument('--sampling_begin_epoch', type=int, default=0)  # pami reply
parser.add_argument('--same_mscale_image', action='store_true', default=False)  # pami reply
parser.add_argument('--st_loss', type=str, default='default')  # pami reply
...
parser.add_argument('--norm_kd_type', type=str, default='default')
...
parser.add_argument('--Dtrans', type=str, default='Dtrans7')
# min lr
...
parser.add_argument('--no_rebn', action='store_true', default=False)
...
def main():
    ...
    # traindir = os.path.join('/mnt/petrelfs/share/imagenet/images', 'train')
    ...
    model = model_lib.Model(res_depth=args.depth, num_classes=numberofclass)
...
def train(train_loader, model, criterion, optimizer, epoch, lr_scheduler):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        if args.norm_kd:  # remove the influence of amplitude of teacher logits
            if args.norm_kd_type=='default':
                max_output = F.layer_norm(max_output, torch.Size((1000,)), None, None, 1e-7)*args.amplitude
            elif args.norm_kd_type=='v1':
                max_output = F.layer_norm(max_output, torch.Size((1000,)), None, None, 1e-7)
            elif args.norm_kd_type=='v2':
                max_output = F.normalize(max_output, p=2.0, dim=1, eps=1e-7)*args.amplitude
            else:
                raise NotImplementedError("norm_kd_type not implemented!")
        loss = torch.mean(criterion(max_output, target))
        max_output_detach = max_output.detach()
        if args.norm_kd_type=='v1':
            adaptive_amplitude = torch.std(max_output_detach, dim=-1, keepdim=True)
        else:
            adaptive_amplitude = 1
        (args.main_coef * loss).backward()  # note：main loss backward
        # import pdb
        # pdb.set_trace()
        # logger.info(args.multi_Snet)
        if args.sampling_begin_epoch <= epoch:
            for _ in range(args.multi_Snet):
                ...
                if args.Snet_Dtrans:
                    ...
                    if args.Dtrans == 'Dtrans4':
                        ...
                        input = Snet_Dtrans(input)
                    elif args.Dtrans == 'Dtrans15':
                        # aux_size = random.randint(96, 224)  # v15
                        aux_size = random.randint(int(args.train_crop_size*args.mini_size), args.train_crop_size)
                        Snet_Dtrans = transforms.Compose([
                            # transforms.Resize((aux_size, aux_size)),
                            transforms.RandomResizedCrop(aux_size),
                            normalize,
                        ])
                        input = Snet_Dtrans(input)
                    else:
                ...
                if args.norm_kd:  # remove the influence of amplitude of student logits
                    if args.norm_kd_type=='default':
                        output = F.layer_norm(output, torch.Size((1000,)), None, None, 1e-7)*args.amplitude
                    elif args.norm_kd_type=='v1':
                        output = F.layer_norm(output, torch.Size((1000,)), None, None, 1e-7)
                    elif args.norm_kd_type=='v2':
                        output = F.normalize(output, p=2.0, dim=1, eps=1e-7)*args.amplitude
                    else:
                        raise NotImplementedError("norm_kd_type not implemented!")

                if args.st_loss == 'default':
                    kl_loss = torch.nn.KLDivLoss(reduction='batchmean')(F.log_softmax(output*adaptive_amplitude/args.Temp, dim=1),
                                            F.softmax(max_output_detach*adaptive_amplitude/args.Temp, dim=1))*args.Temp*args.Temp
                elif args.st_loss == 'CE':  # CE using max_output_detach, equal to KD
                    kl_loss = torch.mean(criterion(output, F.softmax(max_output_detach, dim=1)))
                elif args.st_loss == 'CE_target':  # CE using target, individual supervision for subnet
                    kl_loss = torch.mean(criterion(output, target))
                elif args.st_loss == 'DKD':
                    kl_loss = dkd_loss(output, max_output_detach, target, alpha=1, beta=1, temperature=1)
                elif args.st_loss == 'MSE':
                    kl_loss = torch.nn.MSELoss()(F.softmax(output), F.softmax(max_output_detach))
                else:
                    raise NotImplementedError
        ...
        if args.multi_Snet != 0 and args.sampling_begin_epoch <= epoch:
...
def sample(res_depth=50):
    if res_depth == 50:
        ...
        elif args.sample_space == 'space7':
            depth_candi_v6 = [list(range(1)), list(range(3)), list(range(5)), list(range(1))]
        else:
            raise NotImplementedError
...
target ：标签值
alpha、beta、temperature : 超参数 1, 2.0/0.5, 1
"""
